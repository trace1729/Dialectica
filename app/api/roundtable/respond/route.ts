...
import { roundtableRespondPrompt } from "@/lib/prompts";

export const dynamic = "force-dynamic";

export async function POST(request: NextRequest) {
  try {
    const { philosophers, topic, currentIndex, history } = (await request.json()) as {
      philosophers: string[];
      topic: string;
      currentIndex: number;
    ...
    const systemPrompt = roundtableRespondPrompt(philosophers, topic, currentIndex, history);
